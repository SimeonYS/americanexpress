...
from ..items import AamericanexpressItem
from itemloaders.processors import TakeFirst

pattern = r'(\xa0)?'

class AamericanexpressSpider(scrapy.Spider):
	name = 'americanexpress'
	start_urls = ['http://americanexpress.mediaroom.com/']
	...
	def parse_post(self, response, date):
		...
		item = ItemLoader(item=AamericanexpressItem(), response=response)
